...
function addArtistResponse(container, artist) {
    ...
    card.style.cssText = `padding: 1px;               
    /* borde “neón” */ `;

    // capa interior
    const inner = document.createElement("div");
    inner.className = "rounded-4";
    inner.style.cssText = `background: rgba(255,255,255,.9);
    ...
    imgWrap.style.cssText = `height: 100%;
    ...
    followers.innerHTML = `<span style="font-weight:700; color:#111827;">Escuchas mensuales:</span> ${formatNumber(artist.followers?.total)}`;
    ...
    barWrap.style.cssText = `width:100%; height:10px; background:#e5e7eb; border-radius:999px; overflow:hidden; margin-bottom: .5rem;`;
    const bar = document.createElement("div");
    bar.style.cssText = `width:${pop}%; height:100%; background:${popColor};transition: width .3s ease; border-radius:999px;`;
    ...
    (artist.genres ?? []).slice(0, 6).forEach(g => {
        ...
        badge.style.cssText = `background:#111827; color:white; padding:.5rem .6rem; border-radius:999px;letter-spacing:.2px;`;
    ...
    link.target = "_blank";
    link.textContent = "Escúchalos en Spotify!";
